...
-- Seed with common templates (idempotent: only seed when table is empty)
INSERT INTO notification_templates (name, category, template_text, subject_line)
SELECT * FROM (VALUES
...
) AS seed(name, category, template_text, subject_line)
WHERE NOT EXISTS (SELECT 1 FROM notification_templates);

-- RLS Policies
ALTER TABLE notification_templates ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS notification_templates_admin_only ON notification_templates;
