...
export default function AdminNotificationBell() {
  ...
  const [rewarding, setRewarding]     = useState<string | null>(null)   // id currently showing ✓ anim
  const [loading, setLoading]         = useState(false)
  const panelRef = useRef<HTMLDivElement>(null)
  const btnRef   = useRef<HTMLButtonElement>(null)

  // Load dismissed list and fetch on mount
  useEffect(() => {
  ...
  return (
    <>
      {/* Reward animation keyframes — injected once */}
      <style>{`
      ...
      <div className="relative">
        ...
        <button
          ref={btnRef}
          onClick={toggleOpen}
          aria-label={`Notifications${unreadCount ? ` (${unreadCount} undismissed)` : ''}`}
        ...
        {open && (
          <div
            ref={panelRef}
            className="absolute right-0 top-full mt-2 w-96 rounded-xl border border-neutral-200 bg-white shadow-2xl z-[100] overflow-hidden"
          >
            {/* Header */}
