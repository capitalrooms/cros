...
export default function PropertiesManagementPage() {
  ...
  async function loadProperties() {
    ...
    const { data: roomsData } = await supabase
      .from('rooms')
      .select(
        'id,name,property_id,status,current_asking_rent,previous_rent,tenancies(person_id,end_date,people(id,full_name,email))'
      );
            ...
            {properties.map((property) => (
              <div key={property.id} className="rounded-2xl border-2 border-neutral-200 bg-white overflow-hidden">
                ...
                <div className="bg-neutral-950 text-white px-lg py-md">
                  <div className="flex items-start justify-between gap-md">
                    <div className="flex-1">
                      ...
                      {(property.rooms || []).some((r: any) => r.tenant) && (
                        <p className="mt-sm text-xs text-neutral-300">
                          {(property.rooms || [])
                            .filter((r: any) => r.tenant)
                            .map((r: any) => `${r.name}: ${r.tenant.full_name || r.tenant.email}${r.onNotice ? ' (on notice)' : ''}`)
                            .join(' · ')}
                        </p>
                      )}
                    </div>
                  ...
                  <div className="px-lg py-md bg-neutral-50 border-b border-neutral-200 space-y-lg">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-md">
                        ...
                        <div>
                          <label className="block text-xs text-neutral-700 mb-xs">Portable Appliance Testing (PAT)</label>
                          <input
                            type="date"
                        ...
                        <div>
                          <label className="block text-xs text-neutral-700 mb-xs">Fire Detection Testing</label>
                          <input
                            type="date"
                        ...
                        <div>
                          <label className="block text-xs text-neutral-700 mb-xs">Emergency Lighting Testing</label>
                          <input
                            type="date"
                        ...
                        <div>
                          <label className="block text-xs text-neutral-700 mb-xs">Fire Risk Assessment</label>
                          <input
                            type="date"
                            value={propertyEdits.fire_risk_assessment_date || ''}
                            onChange={(e) =>
                              setPropertyEdits({ ...propertyEdits, fire_risk_assessment_date: e.target.value })
                            }
                            className="w-full rounded border border-neutral-300 px-sm py-xs text-xs"
                          />
                        </div>
                ...
                <div className="px-lg py-md">
                              ...
                              <div className="flex-1">
                                ...
                                {room.tenant && (
                                  <p className="mt-md text-sm text-neutral-700">
                                    ...
                                    {room.onNotice && (
                                      <span className="ml-sm rounded bg-amber-100 px-sm py-xs text-xs font-semibold text-amber-800">on notice</span>
                                    )}
                                  </p>
                                )}
                              </div>
                  ...
                  {/* Deleting a property is rare and destructive — keep it tiny and out of the way. */}
                  <div className="flex justify-end px-lg pb-md" style={{ justifyContent: 'flex-end' }}>
                    <button
            ...
            <div className="rounded-3xl bg-white p-lg max-w-md w-full mx-lg max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-md">
                ...
                <div className="grid grid-cols-2 gap-md">
                  ...
                  {newProperty.property_type === 'hmo' && (
                    <div>
                      <label className="block text-xs font-semibold text-neutral-700 mb-xs">How many rooms?</label>
                      <select
                        value={newProperty.bulkRoomCount}
                        onChange={(e) => setNewProperty({ ...newProperty, bulkRoomCount: Number(e.target.value) })}
                        className="w-full rounded-xl border border-neutral-300 px-md py-md text-base"
                      >
